...
def findPosition(): #TODO: m6 (Fadenabschneider) ansteuern und methode beenden
    ...
    if(GPIO.input(m4_oben, GPIO.HIGH)): #Nadelarm
        GPIO.output(m4, GPIO.HIGH)
        if(GPIO.input(m4_unten, GPIO.HIGH)):
            GPIO.input(m4, GPIO.LOW)

    if(GPIO.input(m3_oben, GPIO.HIGH)): #Fadenrückholer
        GPIO.output(m3, GPIO.HIGH)
        if(GPIO.input(m3_unten, GPIO.HIGH)):
            GPIO.input(m3, GPIO.LOW)
    
    if(GPIO.input(m6_oben, GPIO.HIGH)): #Fadenabschneider TODO: ist noch unklar, ob es auch zwei schalter gibt, ansonsten while schleife wie bei m1
        GPIO.output(m6, GPIO.HIGH)
        if(GPIO.input(m6_unten, GPIO.HIGH)):
